...
GTEST_TEST(ComputeTransportTest, test) {
  auto [sin30, cos30] = std::make_pair(1.0 / 2, sqrt(3) / 2);
  Vector3d t_0, t_1, d1_0, d1_1;
  t_0 << 1.0, 0.0, 0.0;
  t_1 << cos30, sin30, 0.0;
  d1_0 << 0, 4.0 / 5, 3.0 / 5;
  ComputeTransport<double>(t_0, d1_0, t_1, d1_1);
...
GTEST_TEST(ComputeSpaceParallelTransportTest, DidntSpecifyFirstDirector) {
  ...
  Vector3d d1_1_expected;
  ComputeTransport<double>(t.col(0), d1.col(0), t.col(1), d1_1_expected);
  EXPECT_TRUE(CompareMatrices(d1.col(1), d1_1_expected, kTol));
}

GTEST_TEST(ComputeSpaceParallelTransportTest, SpecifyFirstDirector) {
