...
class ExtendedKalmanFilterBase : public GaussianStateObserver<double> {
 ...
 private:
  ...
  // Actuation input port of the observed system (may  be nullptr).
  const InputPort<AutoDiffXd>* observed_system_actuation_input_port_{};
  // Measurement output port of the observed system (will not be nullptr).
 ...
 protected:
  ...
  CalcDynamicsAndLinearize(Context<AutoDiffXd>* observed_system_context,
    ...
    DRAKE_ASSERT(
        !(observed_system_actuation_input_port_ == nullptr ^ u == nullptr));
    if (observed_system_actuation_input_port_) {
      auto u_vec = u->maybe_get_value<BasicVector<double>>();
      if (u_vec) {
        BasicVector<AutoDiffXd> u_vec_ad(u_vec->value());
  ...
  std::pair<Eigen::VectorXd, Eigen::MatrixXd> MeasurementUpdate(
    ...
    auto u =
        this->observed_system_has_actuation_input_port()
