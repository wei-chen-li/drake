...
// Base class for all extended Kalman filterscratch->
class ExtendedKalmanFilterBase : public GaussianStateObserver<double> {
 protected:
  ...
  ExtendedKalmanFilterBase(
    ...
    // observed_system_(.*)put_port_ member variablescratch->
    ...
    // First input port is the output of the observed systescratch->
    ...
    // Setup a chunk of scratch memory as a cache entry.
    scratch_cache_entry_ = &this->DeclareCacheEntry(
        "scratch",
        ValueProducer(Scratch(num_states_, y_size, w_size, v_size),
  ...
  const InputPort<double>& do_get_observed_system_input_input_port()
      const override {
    return this->get_input_port(1);
  }

  const InputPort<double>& do_get_observed_system_output_input_port()
      const override {
    return this->get_input_port(0);
  }

  const OutputPort<double>& do_get_estimated_state_output_port()
      const override {
  ...
  // Returns the scratch stored in the cache in the context.
  Scratch& get_mutable_scratch(const Context<double>& context) const {
    return scratch_cache_entry_->get_mutable_cache_entry_value(context)
        .GetMutableValueOrThrow<Scratch>();
  }

  // Scratch struct holding some preallocated memory.
  struct Scratch {
    Scratch(int x_size, int y_size, int w_size, int v_size) {
  ...
  CalcDynamicsAndLinearize(Context<AutoDiffXd>* observed_system_context,
                           ...
                           const AbstractValue* u, Scratch* scratch) const {
    ...
    // dyn = f(x,u,w=0)
    scratch->set_x_ad(x);
    is_continuous ? observed_system_context->SetContinuousState(scratch->x_ad)
                  : observed_system_context->SetDiscreteState(scratch->x_ad);
    ...
    if (!use_additive_w) {
      observed_system_process_noise_input_port_->FixValue(
          observed_system_context, scratch->w_ad);
    }
    ...
    scratch->dyn_ad =
        is_continuous
            ...
            : observed_system_
                  ...
                  .value();
    if (use_additive_w) {
      scratch->dyn_ad += scratch->w_ad;
    }

    math::ExtractValue(scratch->dyn_ad, &scratch->dyn_value);
    Eigen::Ref<const Eigen::VectorXd> dyn = scratch->dyn_value;

    // A = ∂f/∂x(x,u,w), G = ∂f/∂w(x,u,w).
    math::ExtractGradient(scratch->dyn_ad, std::nullopt,
                          &scratch->dyn_gradient);
    Eigen::Ref<const Eigen::MatrixXd> A =
        scratch->dyn_gradient.leftCols(num_states_);
    Eigen::Ref<const Eigen::MatrixXd> G =
        scratch->dyn_gradient.rightCols(w_size);
  ...
  CalcMeasurementAndLinearize(Context<AutoDiffXd>* observed_system_context,
    ...
    if (!use_additive_v) {
      observed_system_measurement_noise_input_port_->FixValue(
          observed_system_context, scratch->v_ad);
    }

    scratch->y_ad = observed_system_measurement_output_port_->Eval(
        *observed_system_context);
    if (use_additive_v) {
      scratch->y_ad += scratch->v_ad;
    }

    math::ExtractValue(scratch->y_ad, &scratch->y_value);
    Eigen::Ref<const Eigen::VectorXd> y = scratch->y_value;

    // C = ∂g/∂x(x,u,v), H = ∂g/∂v(x,u,v).
    math::ExtractGradient(scratch->y_ad, std::nullopt, &scratch->y_gradient);
    Eigen::Ref<const Eigen::MatrixXd> C =
        scratch->y_gradient.leftCols(num_states_);
    Eigen::Ref<const Eigen::MatrixXd> H = scratch->y_gradient.rightCols(v_size);
  ...
  std::pair<Eigen::VectorXd, Eigen::MatrixXd> MeasurementUpdate(
    ...
    Scratch& scratch = this->get_mutable_scratch(context);
    ...
    auto [yhat, C, H] = CalcMeasurementAndLinearize(
        &this->get_mutable_observed_system_context(context), xhat, u, &scratch);
  ...
  std::pair<Eigen::VectorXd, Eigen::MatrixXd> MeasurementUpdateSqrt(
      const Context<double>& context,
      const Eigen::Ref<const Eigen::VectorXd>& xhat,
      const Eigen::Ref<const Eigen::MatrixXd>& Shat /* ŜŜ' = P̂ */) const {
    ...
    M.topLeftCorner(v_size, y_size) = (H * Vsqrt_).transpose();
    ...
    auto R3 = R.bottomRightCorner(num_states_, num_states_);
 ...
 private:
  ...
  const CacheEntry* scratch_cache_entry_{};
...
// discrete-time measurementscratch->
class ExtendedKalmanFilterDD final : public ExtendedKalmanFilterBase {
 public:
  ...
  void DoSetStateEstimateAndCovariance(
  ...
  // Implements GaussianStateObserver interface.
  Eigen::VectorXd DoGetStateEstimate(
  ...
  // Implements GaussianStateObserver interface.
  Eigen::MatrixXd DoGetStateCovariance(
      const Context<double>& context) const override {
    this->ValidateContext(context);
    if (!use_sqrt_method_) {
  ...
  ExtendedKalmanFilterCont(
    ...
    if (!use_sqrt_method_) {
      this->DeclareContinuousState(
          BasicVector(internal::ConcatenateVectorAndSquareMatrix(
              initial_state_estimate, initial_state_covariance)),
...
// Extended Kalman filter with continuous-time observed system dynamics and
// continuous-time measurementscratch->
