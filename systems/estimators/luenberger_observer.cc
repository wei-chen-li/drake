...
void LuenbergerObserver<T>::CalcEstimatedState(const Context<T>& context,
  ...
  output->set_value(context.has_only_continuous_state()
...
template <typename T>
void LuenbergerObserver<T>::DiscreteUpdate(
    const Context<T>& context, DiscreteValues<T>* x_estimate) const {
  // Ensure observed system context is up to date.
  const Context<T>& observed_system_context =
      observed_system_context_cache_entry_->Eval<Context<T>>(context);

  // Evaluate the observed system measurement.
  const VectorX<T>& yhat =
      observed_system_->get_output_port().Eval(observed_system_context);
  ...
  // x̂[n|n] = x̂[n|n-1] + L(y - g(x̂[n|n-1],u[n])).
  VectorX<T> x_hat = context.get_discrete_state_vector().CopyToVector();
  x_hat = x_hat + observer_gain_ * (y - yhat);

  // x̂[n+1|n] = f_d(x̂[n|n],u[n]).
  ...
  Context<T>& observed_system_context2 =
      cache_entry_value.GetMutableValueOrThrow<Context<T>>();

  observed_system_context2.SetDiscreteState(x_hat);
  x_estimate->SetFrom(observed_system_->EvalUniquePeriodicDiscreteUpdate(
      observed_system_context2));
}
