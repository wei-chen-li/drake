 ...
 **Continuous-time dynamics**
 ...
 @throws std::exception if @p W is not positive definite or if @p V is not
 positive definite.
...
namespace internal {
/* Check if the unscented transform parameters are valid for a given sigma-point
 dimension of `n`. */
void CheckUnscentedTransformParams(
    int n,
    const UnscentedKalmanFilterOptions::UnscentedTransformParameters& params);

/* Computes the unscented transform sigma-points X and weights wₘ, Wc.

 Returns a tuple (X, wₘ, Wc). X is a matrix where each column is a
 sigma-point. wₘ is a vector containing the mean weights, allowing us to
 compute the mean using X * wₘ. Wc is a matrix, allowing us to compute the
 covariance using X * Wc * X.transpose(). */
...
/* An overload of UnscentedTransform() that takes a std::pair for mean and
 covariance. */
std::tuple<Eigen::MatrixXd, Eigen::VectorXd, Eigen::MatrixXd>
UnscentedTransform(
...
/* Calculates the joint mean and covariance given two independently distributed
 Gaussians specified by their mean and covariance. */
