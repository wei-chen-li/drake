...
#include "drake/systems/estimators/test_utilities/sum_matrix_entries_system.h"
...
using estimators_test::SumMatrixEntriesSystem;
...
class DiscreteTimeNonlinearKalmanFilterTest : public ::testing::Test {
 ...
 private:
  ...
  void CheckErrorDynamics(const GaussianStateObserver<double>& observer,
    ...
    auto& A = linear_sys_->A();
    auto& B = linear_sys_->B();
 ...
 protected:
  void TestConstruction(bool use_sqrt_method) {
    ...
    auto observer = MakeObserver(*plant, W_, V_, options);
  ...
  void TestVectorInputErrorDynamics(bool use_sqrt_method) {
    auto plant = linear_sys_;
    Eigen::VectorXd xhat = Eigen::Vector2d::Ones();
    Eigen::MatrixXd Phat(2, 2);
    Phat << 1, 0.5, 0.5, 1;
  ...
  void TestNoInputErrorDynamics(bool use_sqrt_method) {
    DiagramBuilder<double> builder;
    auto sys = builder.AddSystem(linear_sys_);
    auto source = builder.AddSystem<ConstantVectorSource>(0);
    ...
    CheckErrorDynamics(*observer, context.get(), Eigen::VectorXd::Zero(1), y);
  }

  void TestAbstractInputErrorDynamics(bool use_sqrt_method) {
    DiagramBuilder<double> builder;
    auto sys = builder.AddSystem(linear_sys_);
    auto mid = builder.AddSystem<SumMatrixEntriesSystem<double>>(4, 4);
    ...
    Eigen::MatrixXd u = Eigen::Matrix4d::Ones();
    Eigen::VectorXd y(1);
    y << 3.4;
  ...
  void TestExplicitNoiseErrorDynamics(bool use_sqrt_method) {
  ...
  void TestSteadyState(bool use_sqrt_method) {
    ...
    options.initial_state_estimate = xhat;
    options.initial_state_covariance = Phat;
    auto observer2 = MakeObserver(*linear_sys_, W_, V_, options);

    DiagramBuilder<double> builder;
    auto observer =
        builder.AddSystem(MakeObserver(*linear_sys_, W_, V_, options));
    auto source =
...
class ContinuousDiscreteNonlinearKalmanFilterTest : public ::testing::Test {
 ...
 protected:
  void TestConstruction(bool use_sqrt_method) {
    ...
    options.initial_state_covariance = Phat;
    auto observer =
        MakeObserver(*plant, W_, V_, measurement_time_period_, options);
  ...
  void TestSimulation(bool use_sqrt_method) {
    ...
    DiagramBuilder<double> builder;
    auto observer = builder.AddSystem(
        MakeObserver(*linear_sys_, W_, V_, measurement_time_period_, options));
