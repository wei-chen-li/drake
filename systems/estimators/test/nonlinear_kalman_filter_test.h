...
class DiscreteTimeNonlinearKalmanFilterTest
 ...
 protected:
  void TestConstruction(bool use_sqrt_method = false) {
  ...
  void TestVectorInputDynamics(bool use_sqrt_method = false) {
    auto plant = linear_sys_;
    auto options = this->MakeOptions(use_sqrt_method);
    auto observer = this->MakeObserver(*plant, W_, V_, options);
    auto context = observer->CreateDefaultContext();
  ...
  void TestNoInputDynamics(bool use_sqrt_method = false) {
  ...
  void TestAbstractInputDynamics(bool use_sqrt_method = false) {
    DiagramBuilder<double> builder;
    auto sys = builder.AddSystem(linear_sys_);
  ...
  void TestProcessNoiseInputDynamics(bool use_sqrt_method = false) {
  ...
  void TestMeasurementNoiseInputDynamics(bool use_sqrt_method = false) {
  ...
  void TestProcessAndMeasurementNoiseInputDynamics(
      bool use_sqrt_method = false) {
  ...
  void TestSteadyState(bool use_sqrt_method = false) {
    Eigen::VectorXd xhat = Eigen::Vector2d::Ones();
    Eigen::MatrixXd Phat(2, 2);
    Phat << 1, 0.5, 0.5, 1;
    auto options = this->MakeOptions(use_sqrt_method);
...
class ContinuousDiscreteNonlinearKalmanFilterTest
 ...
 protected:
  ...
  void TestSimulation(bool use_sqrt_method = false) {
    Eigen::VectorXd xhat = Eigen::Vector2d::Ones();
    Eigen::MatrixXd Phat(2, 2);
    Phat << 1, 0.5, 0.5, 1;
    auto options = this->MakeOptions(use_sqrt_method);
