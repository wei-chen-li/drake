...
GTEST_TEST(ConcatenateVectorAndMatrixTest, SquareMatrix) {
  Eigen::VectorXd vec(2);
  Eigen::MatrixXd mat(2, 2);
  vec << 1, 2;
  mat << 3, 4, 5, 6;

  Eigen::VectorXd concat = ConcatenateVectorAndSquareMatrix(vec, mat);
  Eigen::VectorXd expected(6);
  expected << 1, 2, 3, 5, 4, 6;
  EXPECT_TRUE(CompareMatrices(concat, expected));

  Eigen::MatrixXd mat_out(2, 2);
  ExtractSquareMatrix(concat, mat_out);
  EXPECT_TRUE(CompareMatrices(mat, mat_out));
}

GTEST_TEST(ConcatenateVectorAndMatrixTest, LowerTriMatrix) {
  Eigen::VectorXd vec(2);
  Eigen::MatrixXd mat(2, 2);
  vec << 1, 2;
  mat << 3, 0, 4, 5;
  ...
  Eigen::VectorXd concat = ConcatenateVectorAndLowerTriMatrix(vec, mat);
  Eigen::VectorXd expected(5);
  expected << 1, 2, 3, 4, 5;
  EXPECT_TRUE(CompareMatrices(concat, expected));

  // Overload taking triangular view.
  Eigen::VectorXd concat2 = ConcatenateVectorAndLowerTriMatrix(
      vec, mat.template triangularView<Eigen::Lower>());
  EXPECT_TRUE(CompareMatrices(concat2, expected));

  Eigen::MatrixXd mat_out(2, 2);
  ExtractLowerTriMatrix(concat, mat_out);
  EXPECT_TRUE(CompareMatrices(mat, mat_out));
}
