  ...
  for (double h : time_steps) {
    for (int i = 0; i < 2; ++i) {
      ...
      if (h == 0.0) {
        ...
        Eigen::VectorXd x_est = xhat + L * (y - C * xhat - D * u);
        x_est = A * x_est + B * u;
        ...
        auto events = observer->AllocateCompositeEventCollection();
        observer->GetPeriodicEvents(*context, events.get());

        auto status = observer->CalcDiscreteVariableUpdate(
            *context, events->get_discrete_update_events(),
