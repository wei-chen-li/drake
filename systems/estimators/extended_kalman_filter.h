...
/* Concatenate `vector` with the vectorized `square_matrix`. */
...
/* Concatenate `vector` with the vectorized `lower_tri_matrix`. */
...
/* Extract the `square_matrix` from the `concatenated` vector returned by
...
/* Extract the `lower_tri_matrix` from the `concatenated` vector returned by
...
/*
 ...
 repeat and have the correct attributes.

 @param[in] observed_system The observed system.
 @param[in] options For port specification.
 @param[out] observed_system_actuation_input_port Pointer to the actuation
 input port (may be nullptr indicating no actuation input).
 @param[out] observed_system_measurement_output_port Pointer to the
 measurement output port (will not be nullptr).
 @param[out] observed_system_process_noise_input_port Pointer to the
 process noise output port (may be nullptr indicating additive noise).
 @param[out] observed_system_measurement_noise_input_port Pointer to the
...
void CheckObservedSystemInputOutputPorts(
  ...
  DRAKE_ASSERT(observed_system_actuation_input_port != nullptr);
  DRAKE_ASSERT(observed_system_measurement_output_port != nullptr);
  DRAKE_ASSERT(observed_system_process_noise_input_port != nullptr);
  DRAKE_ASSERT(observed_system_measurement_noise_input_port != nullptr);
