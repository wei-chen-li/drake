 ...
 * input port (will not be nullptr).
...
void CheckObservedSystemInputOutputPorts(
  ...
  if (actuation_input_port->get_data_type() == kAbstractValued) {
    throw std::logic_error(
        "The specified input port is abstract-valued, but Kalman filter only "
        "supports vector-valued input ports.  Did you perhaps forget to pass a "
        "non-default `actuation_input_port_index` argument?");
  }
