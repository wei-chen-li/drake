...
// [Sarkka, 2007] S. Sarkka, "On unscented Kalman filtering for state estimation
// of continuous-time nonlinear systems," IEEE Transactions on Automatic
// Control, vol. 52, no. 9, pp. 1631-1641, 2007.
// [Barfoot, 2017] T. D. Barfoot, State Estimation for Robotics, Cambridge
...
class UnscentedKalmanFilterBase : public GaussianStateObserver<double> {
 protected:
  ...
  Eigen::MatrixXd BatchCalcDynamics(
      Context<double>* observed_system_context,
      const Eigen::Ref<const Eigen::MatrixXd>& xs, const AbstractValue* u,
      std::optional<Eigen::Ref<const Eigen::MatrixXd>> ws =
          std::nullopt) const {
    DRAKE_ASSERT(!ws || xs.cols() == ws->cols());
    Eigen::MatrixXd dyns(xs.rows(), xs.cols());
    for (int i = 0; i < xs.cols(); ++i) {
      dyns.col(i) =
          !ws ? CalcDynamics(observed_system_context, xs.col(i), u)
              : CalcDynamics(observed_system_context, xs.col(i), u, ws->col(i));
    }
    return dyns;
  }
  ...
  Eigen::MatrixXd BatchCalcMeasurement(
      Context<double>* observed_system_context,
      const Eigen::Ref<const Eigen::MatrixXd>& xs, const AbstractValue* u,
      std::optional<Eigen::Ref<const Eigen::MatrixXd>> vs =
          std::nullopt) const {
    DRAKE_ASSERT(!vs || xs.cols() == vs->cols());
    const int y_size = observed_system_measurement_output_port_->size();
    Eigen::MatrixXd ys(y_size, xs.cols());
    for (int i = 0; i < xs.cols(); ++i) {
      ys.col(i) = !vs ? CalcMeasurement(observed_system_context, xs.col(i), u)
                      : CalcMeasurement(observed_system_context, xs.col(i), u,
                                        vs->col(i));
    }
    return ys;
  }
  ...
  std::pair<Eigen::VectorXd, Eigen::MatrixXd> MeasurementUpdate(
    ...
    if (!observed_system_has_measurement_noise_input_port()) {
      ...
      mu_y = Y * wm;
      Sigma_yy = Y * Wc * Y.transpose() + V_;
      Sigma_xy = X * Wc * Y.transpose();
    } else {
      ...
      auto X = Z.topRows(num_states_);
      auto Y = BatchCalcMeasurement(
          &this->get_mutable_observed_system_context(context), X, u,
          Z.bottomRows(v_size));

      mu_y = Y * wm;
      Sigma_yy = Y * Wc * Y.transpose();
      Sigma_xy = X * Wc * Y.transpose();
    }

    Eigen::MatrixXd K = Sigma_xy * Sigma_yy.inverse();
    auto Phat_new = Phat - K * Sigma_xy.transpose();
    auto xhat_new = xhat + K * (y - mu_y);
 ...
 private:
  ...
  std::pair<Eigen::VectorXd, Eigen::MatrixXd> ProcessUpdate(
    ...
    if (!observed_system_has_process_noise_input_port()) {
      ...
      auto Xnext = BatchCalcDynamics(
          &this->get_mutable_observed_system_context(context), X, u);

      auto xhat_next = Xnext * wm;
      auto Phat_next = Xnext * Wc * Xnext.transpose() + W_;
      ...
      auto xhat_next = Xnext * wm;
      auto Phat_next = Xnext * Wc * Xnext.transpose();
      return {xhat_next, Phat_next};
    }
  }
};
 ...
 private:
  ...
  void DoCalcTimeDerivatives(
    ...
    if (!observed_system_has_process_noise_input_port()) {
      ...
      auto GWG = Xdot * Wc * Xdot.transpose() -
...
UnscentedTransform(
  ...
  Eigen::VectorXd wm = 0.5 / (n + lambda) * Eigen::VectorXd::Ones(num_points);
  ...
  Eigen::MatrixXd I_minus_wm =
      Eigen::MatrixXd::Identity(num_points, num_points).colwise() - wm;
  Eigen::MatrixXd Wc = I_minus_wm * wc.asDiagonal() * I_minus_wm.transpose();
