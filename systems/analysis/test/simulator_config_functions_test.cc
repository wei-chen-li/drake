...
TYPED_TEST(IntegratorConfigFunctionsTest,
           IsIntegrationSchemeSupportedScalarTypeTest) {
  using T = TypeParam;
  for (const auto& [scheme, type_name, support_symbolic] : this->suites_) {
    if constexpr (std::is_same_v<T, symbolic::Expression>) {
      EXPECT_EQ(IsIntegrationSchemeSupportedScalarType<T>(scheme),
                support_symbolic);
    } else {
      EXPECT_TRUE(IsIntegrationSchemeSupportedScalarType<T>(scheme));
    }
  }
  DRAKE_EXPECT_THROWS_MESSAGE(IsIntegrationSchemeSupportedScalarType<T>("abc"),
